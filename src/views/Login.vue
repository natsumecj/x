<template>
    <div>
        <div class="bg">
            <el-card class="box-card">
                <div slot="header" class="clearfix">
                    欢迎来到小金刚后台管理系统
                </div>
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="110px" label-position="left" class="demo-ruleForm">
                    <el-form-item label="请输入用户名" prop="username">
                        <el-input v-model="ruleForm.username" clearable :minlengt="mini" class="user"></el-input>
                    </el-form-item>
                    <el-form-item label="请输入密码" prop="password">
                        <el-input type="password" v-model="ruleForm.password" clearable></el-input>
                        <el-button type="success" :loading="ruleForm.load" class="btn" @click="forget">忘记密码</el-button>
                    </el-form-item>
                    <el-form-item label="请输入验证码" prop="code" class="code">
                        <el-input v-model="ruleForm.code" clearable></el-input>
                        <span v-html="cap" class="cap" @click="change"></span>
                    </el-form-item>
                    <el-form-item>
                        <el-button type="primary" class="btn2" @click="login">立即登录</el-button>
                        <el-button type="primary" @click="register">立即注册</el-button>
                    </el-form-item>
                </el-form>
            </el-card>
        </div>
    </div>
</template>

<script>
    export default {
        name: "Login",
        data(){
            return{
                mini:2,
                cap:'',
                ruleForm: {
                    username: '',//用户名
                    password: '',//密码
                    code: '',//验证码
                    load:false,//忘记密码的按钮加载条件
                },
                rules: {
                    username: [
                        { required: true, message: '请输入用户名', trigger: 'blur' },
                        { min: 2, max: 10, message: '长度在 2 到 10 个字符', trigger: 'blur' }
                    ],
                    password: [
                        { required: true, message: '请输入密码', trigger: 'blur' },
                        { min: 6, max: 18, message: '长度在 6 到 18 个字符', trigger: 'blur' }
                    ],
                    code:[
                        { required: true, message: '请输入验证码', trigger: 'blur' },
                    ]
                }
            }
        },
        methods:{
            forget(){
                this.$router.push('./forget')
            },
            register(){
                this.$router.push('./register')
            },
            change(){
                this.$axios.req('api/users/captcha')
                    .then(res=>{
                        console.log(res);
                        this.cap=res.data
                    })
                    .catch(err=>{
                        console.log(err);
                    })
            },
            login(){
                this.$axios.req('api/users/login',{
                    username:this.ruleForm.username,
                    password:this.ruleForm.password,
                    code:this.ruleForm.code
                })
                    .then(res=>{
                        console.log(res);
                        if(res.data.code===200){
                            this.$message({
                                type:'success',
                                message:res.data.msg
                            })
                            localStorage.setItem('user',res.data.data.username)
                            localStorage.setItem('userId',res.data.data._id)
                            this.$store.state.user=res.data.data.username
                            this.$router.push('/')
                        }
                        if(res.data.code===500){
                            this.$message({
                                type:'warning',
                                message:res.data.msg
                            })
                            this.ruleForm.username=''
                            this.ruleForm.password=''
                        }
                    })
                    .catch(err=>{
                        console.log(err);
                    })
            }
        },
        mounted() {
            this.$axios.req('api/users/captcha')
                .then(res=>{
                    console.log(res);
                    this.cap=res.data
                })
                .catch(err=>{
                    console.log(err);
                })
        }
    }
</script>

<style scoped lang="scss">
    .bg{
        background-image: url("../assets/8718367adab44aed29d58cb7b21c8701a08bfbcd.jpg");
        width: 100%;
        height: 100%;
        /*background-size:100%;*/
        position: absolute;
        top:0;
        left: 0;
        right: 0;
        bottom: 0;
    }
    .clearfix{
        text-align: center;
    }
    .box-card{
        width: 600px;
        margin: auto;
        margin-top: 100px;
    }
    .btn{
        margin-left: 30px;
    }
    .btn2{
        margin-right: 100px;
    }
    .el-input{
        width: 240px;
    }
    .user{
        width: 340px;
    }
    .cap{
        position: absolute;
        top:-10px;
        left: 270px;
    }
</style>